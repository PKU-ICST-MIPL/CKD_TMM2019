...
def build_net(ntype, nin, nwb=None, name=None, shape=None, out_shape=None, need_relu=True):
	...
	if ntype=='conv':
		...
		if nwb == None:
			...
			nwb.append(tf.get_variable(name=name+'_W', shape=shape, initializer=W_init, **W_init_args))
			nwb.append(tf.get_variable(name=name+'_b', shape=(shape[-1]), initializer=b_init, **b_init_args))
		return tf.nn.relu(tf.matmul(nin, nwb[0]) + nwb[1], name=name)
	elif ntype=='upsample':
...
def main():
	...
	with tf.variable_scope(tf.get_variable_scope()):
		...
		G_loss = 0
		G_loss += compute_error(vgg_net['input'], g_net['g_img']) * 1
		G_loss += compute_error(vgg_net['conv1_1'], g_net['g_conv1_1']) * 1
		G_loss += compute_error(vgg_net['conv1_2'], g_net['g_conv1_2']) * 1
		G_loss += compute_error(vgg_net['pool1'], g_net['g_pool1']) * 1
		
		G_loss += compute_error(vgg_net['conv2_1'], g_net['g_conv2_1']) * 1
		G_loss += compute_error(vgg_net['conv2_2'], g_net['g_conv2_2']) * 1
		G_loss += compute_error(vgg_net['pool2'], g_net['g_pool2']) * 1
		
		G_loss += compute_error(vgg_net['conv3_1'], g_net['g_conv3_1']) * 1
		G_loss += compute_error(vgg_net['conv3_2'], g_net['g_conv3_2']) * 1
		G_loss += compute_error(vgg_net['conv3_3'], g_net['g_conv3_3']) * 1
		G_loss += compute_error(vgg_net['conv3_4'], g_net['g_conv3_4']) * 1
		G_loss += compute_error(vgg_net['pool3'], g_net['g_pool3']) * 1
		
		G_loss += compute_error(vgg_net['conv4_1'], g_net['g_conv4_1']) * 1
		G_loss += compute_error(vgg_net['conv4_2'], g_net['g_conv4_2']) * 1
		G_loss += compute_error(vgg_net['conv4_3'], g_net['g_conv4_3']) * 1
		G_loss += compute_error(vgg_net['conv4_4'], g_net['g_conv4_4']) * 1
		G_loss += compute_error(vgg_net['pool4'], g_net['g_pool4']) * 1
		
		G_loss += compute_error(vgg_net['conv5_1'], g_net['g_conv5_1']) * 1
		G_loss += compute_error(vgg_net['conv5_2'], g_net['g_conv5_2']) * 1
		G_loss += compute_error(vgg_net['conv5_3'], g_net['g_conv5_3']) * 1
		G_loss += compute_error(vgg_net['conv5_4'], g_net['g_conv5_4']) * 1
		G_loss += compute_error(vgg_net['pool5'], g_net['g_pool5_reshape']) * 1
	...
	for epoch in range(1, MAX_EPOCH + 1):
		...
		for index in random_index:
			...
			_, G_current = sess.run([G_opt, G_loss], feed_dict={input_image: image_batch_narray, input_text: text_batch_narray, lr: LEARNING_RATE})					
